...
auto maximum_average(const int& n, const std::vector<gnt>& sums, const int& k) {
    fraction result(sums[k - 1], k);
    for (int i{k}; i < n; ++i) {
        fraction current(sums[i] - sums[i - k], k); current.simplify();
...
auto solve(const int& n, const int& l, const std::vector<lli>& v) {
    std::vector<gnt> sums(begin(v), end(v));
    for (int i{1}; i < n; ++i) {
        sums[i] += sums[i - 1];
    }
    ...
    fraction max(-1, 1);
    for (int k{l}; k <= n; ++k) {
        max = std::max(max, maximum_average(n, sums, k));
    }
