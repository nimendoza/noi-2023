...
auto bfs(
    ...
    for (int neighbor : neighbors[source]) {
        if (neighbor != parent) {
            best += (
                take ? bfs(neighbors, neighbor, source, false, false) 
                : std::max(
                    bfs(neighbors, neighbor, source, true, false),
                    bfs(neighbors, neighbor, source, false, false)
                )
            );
        }
    }
...
auto solve(const int& n, const vint& parent) {
    ...
    return std::max(
        bfs(neighbors, 0, parent[0], true, true),
        bfs(neighbors, 0, parent[0], false, true)
    );
}
