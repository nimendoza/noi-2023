...
int main(int argc, char** argv) {
    ...
    auto W{get_generator(engine, -parse(argv[3]), parse(argv[3]))};
    auto K{get_generator(engine, 1, parse(argv[4]))};
    auto A{get_generator(engine, -parse(argv[5]), parse(argv[5]))};

    gnt k{K(engine)};

    auto F{get_generator(engine, 1, k)};

    std::cout << 1 << '\n';
    std::cout << N(engine) << ' ' << W(engine) << ' ' << k << ' ' << F(engine) << '\n';

    for (gnt i{}; i < k; ++i) {
        std::cout << A(engine) << (i == k - 1 ? '\n' : ' ');
    }
}
